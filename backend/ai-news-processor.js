...
class AINewsProcessor {
    ...
    generateIntelligentSummary(article) {
        ...
        if (entities.people.length > 0) {
            keyPoints.push(`👤 Who: ${entities.people.slice(0, 2).join(', ')}`);
        } else {
            ...
            for (const pattern of whoPatterns) {
                ...
                if (match) {
                    keyPoints.push(`👤 Who: ${match[0].substring(0, 50)}`);
                    break;
                }
            }
        }
        ...
        if (entities.places.length > 0) {
            keyPoints.push(`📍 Where: ${entities.places.slice(0, 2).join(', ')}`);
        }

        // WHEN - Improved time extraction
        if (entities.dates.length > 0) {
            keyPoints.push(`📅 When: ${entities.dates[0]}`);
        } else {
            const timePatterns = [
        ...
        for (const pattern of whyPatterns) {
            const match = text.match(pattern);
            if (match) {
                keyPoints.push(`❓ Why: ${match[1].trim()}`);
                break;
            }
        }
        ...
        for (const pattern of howPatterns) {
            const match = text.match(pattern);
            if (match) {
                keyPoints.push(`🔧 How: ${match[1].trim()}`);
                break;
            }
        }
        ...
        if (impact.level !== 'low') {
            keyPoints.push(`⚡ Impact: ${impact.description}`);
        }
    ...
    extractEntities(article) {
        ...
        const moneyPatterns = [
            /(?:Rs\.?|₹)\s*(\d+(?:,\d+)*(?:\.\d+)?)\s*(?:crore|lakh|thousand|million|billion|trillion)?/gi,
    ...
    analyzeSource(article) {
        ...
        if (sourceInfo) {
            return {
                ...
                badge: trustedSources[sourceInfo].tier === 1 ? '🏆 Premium Source' : '✅ Verified Source'
            };
        }
